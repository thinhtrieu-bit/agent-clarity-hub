...
function mapEmail(row: any): EmailActivity {
  return {
    id: row.id,
    subject: row.subject,
    from: row.sender,
...
export async function getSnapshot(): Promise<DashboardSnapshot> {
  const [agentsRes, tasksRes, messagesRes, emailsRes, eventsRes] = await Promise.all([
    ...
    supabase.from("emails").select("*").order("timestamp", { ascending: false }),
    supabase.from("events").select("*").order("timestamp", { ascending: false }).limit(30),
  ]);
